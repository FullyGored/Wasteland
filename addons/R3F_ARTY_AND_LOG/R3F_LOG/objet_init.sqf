...
if ({_objet isKindOf _x} count R3F_LOG_CFG_objets_deplacables > 0) then
{
	...
	_objet addAction [("<t color=""#CC0000"">" + STR_CHECK_HEALTH + "</t>"), "addons\R3F_ARTY_AND_LOG\R3F_LOG\objet_deplacable\objectHealth.sqf", nil, -6, false, true,"","R3F_LOG_objet_addAction == _target && R3F_LOG_action_deplacer_objet_valide && R3F_LOG_action_hpcheck_object_valid"];
	_objet addAction [("<t color=""#21DE31"">" + STR_REPAIR_OBJECT + "</t>"), "addons\R3F_ARTY_AND_LOG\R3F_LOG\objet_deplacable\objectRepair.sqf", nil, -6, false, true,"","R3F_LOG_objet_addAction == _target && R3F_LOG_action_deplacer_objet_valide && R3F_LOG_action_repair_object_valid"];
};
