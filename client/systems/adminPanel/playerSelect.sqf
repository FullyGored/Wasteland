...
private ["_dialog","_playerListBox","_spectateButton","_switch","_index","_modSelect","_playerData","_target","_check","_spectating","_camadm","_rnum","_warnText","_targetUID","_playerName","_gcamViewer"];
...
if ((_uid in moderatorsFF) OR (_uid in administratorsFF) OR (_uid in serverAdministratorsFF)) then {
	...
	switch (_switch) do
	{
        ...
        case 8: //Spectate with G Cam
	    {
                        _gcamViewer = [ _target ] execVM "addons\gcam\gcam.sqf";
	    };
	};
} else {
  exit;
