...
_currSpawnBeaconFaction = ((nearestobjects [getpos player, ["Satelit"],  10] select 0) getVariable "faction");
...
_currSpawnBeacon = (nearestobjects [getpos player, ["Satelit"],  10] select 0);
...
switch (_destroyOrSteal) do {
    case 0:{ // Steal
		...
		for "_iteration" from 1 to _lockDuration do {
			...
			if(player distance _currSpawnBeacon > 10) exitWith { // If the player leaves, revert state.
				2 cutText ["Steal spawn beacon interrupted...", "PLAIN DOWN", 1];
		        mutexScriptInProgress = false;
			};
			...
			if (_iteration >= _totalDuration) exitWith { // Sleep a little extra to show that steal has completed.
                ...
                _currBeaconTemp = (nearestObjects [getpos player, ["Satelit"],  10]);
		        ...
		        if(count _currBeaconTemp == 0) then { // Check if the beacon has been removed since curr player started interacting with it.
                    ...
                    mutexScriptInProgress = false;
	               	...
	               	deleteVehicle (nearestobjects [getpos player, ["Satelit"],  10] select 0);
	                ...
	                [_currBeaconOwnerUID] execVM "client\functions\cleanBeaconArrays.sqf"; // Now that the previous instance of the spawn beacon has been technically removed, remove it from the spawn locations list.
                        diag_log_msg = format["%1's spawn beacon has been STOLEN by %2",_currBeaconOwnerName, name player];
				...
				sleep 1;
		                ...
		                if(count _currBeaconTemp == 0) then { // Check if the beacon has been removed since curr player started interacting with it.
			                 ...
			                 mutexScriptInProgress = false;
	                                 ...
	                                 [_currBeaconOwnerUID] execVM "client\functions\cleanBeaconArrays.sqf"; // Now that the previous instance of the spawn beacon has been technically removed, remove it from the spawn locations list.
                                         diag_log_msg = format["%1's spawn beacon has been DESTROYED by %2",_currBeaconOwnerName, name player];
                                         pvar_clientDiagLog = [diag_log_msg];
                                         publicVariableServer "pvar_clientDiagLog";
                                         {
                                            if(getPlayerUID _x == _currBeaconOwnerUID) then {
