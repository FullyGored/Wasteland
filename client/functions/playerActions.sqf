...
aActionsIDs = aActionsIDs + [player addAction["Take Canned food", "noscript.sqf", 'deleteVehicle (nearestobjects [player, ["Land_Bag_EP1"],  5] select 0); player setVariable["canfood", (player getVariable "canfood")+1,true]; hint "You have taken some canned food."; player playmove "AinvPknlMstpSlayWrflDnon"',0,false,false,"",'player distance (nearestobjects [player, ["Land_Bag_EP1"],  5] select 0) < 5 and (player getVariable "canfood")<3']];
...
aActionsIDs = aActionsIDs + [player addAction[("<t color=""#E01B1B"">Destroy spawn beacon</t>"), "client\actions\pickupBeacon.sqf", 1, 1, false, false, "", 'player distance (nearestobjects [player, ["Satelit"],  10] select 0) < 10']];
aActionsIDs = aActionsIDs + [player addAction[("<t color=""#21DE31"">Steal spawn beacon</t>"), "client\actions\pickupBeacon.sqf", 0, 1, false, false, "", 'player distance (nearestobjects [player, ["Satelit"],  10] select 0) < 10']];

//Detect Nearest SpawnBeacon (Satelit)
aActionsIDs = aActionsIDs + [player addAction[("<t color=""#21DE31"">Beacon Detector</t>"), "client\actions\detectBeacon.sqf", 1, 1, false, false, "", 'player distance (nearestobjects [player, ["Satelit"],  750] select 0) < 750 AND ((nearestobjects [player, ["Satelit"],  750] select 0) getVariable "faction") != "WORLD"']];
...
//Right flipped vehicles
aActionsIDs = aActionsIDs + [player addAction[("<t color=""#21DE31"">Right Overturned Vehicle</t>"), "client\actions\unflipVehicle.sqf", [], 7, true, false, "", 'player distance (nearestobjects [player, ["Car","Motorcycle","Tank"], 10] select 0) < 10 AND (count (crew (nearestobjects [player, ["Car","Motorcycle","Tank"], 10] select 0)))== 0 AND ((vectorUp (nearestobjects [player, ["Car","Motorcycle","Tank"], 10] select 0)) select 2) < 0.5 ']];

//Bad idea for Fast Ropes action :(
//aActionsIDs = aActionsIDs + [player addAction[("<t color=""#ED2744"">Fast rope</t>", "client\actions\BTC_fast_rope.sqf", [], 6, false, false, "", '_veh = vehicle player; player != _veh; ({_veh isKindOf _x} count ropingChoppers > 0); player != driver _veh; ((getPos _veh) select 2) < 20 AND speed _veh < 5']];
