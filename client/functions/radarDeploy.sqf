...
private["_radarTank","_player", "_stringEscapePercent", "_totalDuration", "_lockDuration", "_iteration", "_playerSide", "_playerUID", "_playerPos", "_radarStation", "_radarStationPos", "_uniqueID", "_temp", "_markerState", "_tankFuel", "_tankDamage"];
...
for "_iteration" from 1 to _actionDuration do {
	...
	if (_iteration >= _totalDuration) exitWith { // Sleep a little extra to show that place has completed.
        ...
        _command = format["if isServer then {this setVariable [""PG_result"",[call {[%1,%2,%3,%4,%5] execVM 'server\functions\radarMarkerUpdate.sqf'}],true]}",str(_uniqueID),str(_radarStationPos),str(_playerSide),_markerState,str(_playerUID)];
